    ...
    <div className="flex w-full justify-center py-10">
      <div className="max-w-screen-xl md:px-12">
        <h2 className="mb-14 flex flex-col gap-2">
          ...
          <span className="text-subheading">Love and trusted by content creators, digital marketers, and more</span>
        </h2>
        <div id="testimonial-list" className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {comments?.map((comment, index) => (
            <div
              key={index}
              className="flex h-full flex-col rounded-lg border border-neutral-300  p-4"
              style={{ minHeight: '250px' }} // Set a minimum height
            >
              <h4 className="m-2 flex-grow flex-wrap text-ellipsis text-heading font-normal ">{formatText(comment.text)}</h4>
              <div className="mx-4 flex flex-row items-center gap-4">
                {comment.img ? (
                  ...
                  <div className="relative h-10 w-10 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-600">
                    <svg className="absolute -bottom-1 h-6 w-6 " fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
